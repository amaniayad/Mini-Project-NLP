...
def save_note_to_file(response):
    with open("note.txt", "a") as file:
        file.write(f"A: {response}\n\n")
    st.success("Note saved successfully!")

def handle_userinput(user_question):
    if "write a note" in user_question.lower():
        if st.session_state.chat_history:
            last_response = st.session_state.chat_history[-1].content
            save_note_to_file(last_response)
            st.write(bot_template.replace("{{MSG}}", "Note saved successfully!"), unsafe_allow_html=True)
        else:
            st.write(bot_template.replace("{{MSG}}", "No previous response to save."), unsafe_allow_html=True)
        return
    ...
    for i, message in enumerate(st.session_state.chat_history):
        if i % 2 == 0:
            ...
            if validate_answer_against_sources(message.content, response['source_documents']):
                st.write(bot_template.replace(
                    "{{MSG}}", message.content), unsafe_allow_html=True)
            else:
                st.write(bot_template.replace(
                    "{{MSG}}", "I cannot provide an answer based on the provided document."), unsafe_allow_html=True)
